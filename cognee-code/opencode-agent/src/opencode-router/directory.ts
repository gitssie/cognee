...
const SOURCE_TEMPLATE_DIR = resolve(MODULE_DIR, "workspace-template");
...
export async function provisionPeerDirectory(
  ...
  // Copy built-in template files — skip if missing (handled by workspace-init.ts)
  try {
    const templateDir = await resolveBuiltinTemplateDir();
    try { await stat(templateDir); } catch { return peerDir; }
