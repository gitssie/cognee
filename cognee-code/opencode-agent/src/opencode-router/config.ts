 ...
 */
export function parseDirectoryStrategy(
    value: string | undefined | null,
): DirectoryStrategy | undefined {
    ...
    for (const [key, value] of Object.entries(entries)) {
        ...
        normalized[key.trim()] = {
            ...(typeof record.enabled === "boolean"
                ...
                : {}),
            ...(record.hooks &&
            typeof record.hooks.allowPromptInjection === "boolean"
                ? {
                      hooks: {
                          allowPromptInjection:
                              record.hooks.allowPromptInjection,
                      },
                  }
                : {}),
            ...(record.config &&
...
function normalizePluginSpecEntry(
    entry: unknown,
    configPath: string,
...
function normalizePluginOrigins(
    file: OpenCodeRouterConfigFile,
    configPath: string,
): PluginOrigin[] {
    ...
    for (const spec of pluginsConfig.loadPaths) {
        ...
        if (!key || seen.has(key)) continue;
        seen.add(key);
        const legacyConfig = legacyPluginConfigForSpec(
            resolvedSpec,
...
function normalizePluginsConfig(file: OpenCodeRouterConfigFile) {
    ...
    return {
        ...
        deny: Array.isArray(plugins?.deny)
            ? plugins!.deny.map((entry) => String(entry).trim()).filter(Boolean)
            : [],
        loadPaths: Array.isArray(plugins?.load?.paths)
            ? plugins!
                  .load!.paths!.map((entry) => String(entry).trim())
                  .filter(Boolean)
            : [],
...
export function readConfigFile(configPath: string): {
    exists: boolean;
    config: OpenCodeRouterConfigFile;
} {
    try {
...
function coerceTelegramBots(
    file: OpenCodeRouterConfigFile,
): TelegramIdentity[] {
    const telegram = file.channels?.telegram;
    const bots = Array.isArray((telegram as any)?.bots)
        ? ((telegram as any).bots as unknown[])
        : [];
    ...
    for (const entry of bots) {
        ...
        normalized.push({
            ...
            token,
            enabled:
                record.enabled === undefined ? true : record.enabled === true,
            ...(directory ? { directory } : {}),
            ...(access === "private"
                ? { access, ...(pairingCodeHash ? { pairingCodeHash } : {}) }
                : { access: "public" }),
        });
    }
    ...
    const legacyToken =
        typeof (telegram as any)?.token === "string"
            ? String((telegram as any).token).trim()
            : "";
    if (legacyToken) {
...
function coerceSlackApps(file: OpenCodeRouterConfigFile): SlackIdentity[] {
    ...
    const apps = Array.isArray((slack as any)?.apps)
        ? ((slack as any).apps as unknown[])
        : [];
    ...
    const legacyBot =
        typeof (slack as any)?.botToken === "string"
            ? String((slack as any).botToken).trim()
            : "";
    const legacyApp =
        typeof (slack as any)?.appToken === "string"
            ? String((slack as any).appToken).trim()
            : "";
 ...
 */
function coerceChannelAccounts(
    file: OpenCodeRouterConfigFile,
): ChannelIdentity[] {
    ...
    for (const [channelName, channelCfg] of Object.entries(channels)) {
        ...
        const accountList = Array.isArray(cfg.accounts)
            ? cfg.accounts
            : Array.isArray(cfg.bots)
              ? cfg.bots
              : Array.isArray(cfg.apps)
                ? cfg.apps
                : [];
        ...
        if (accountList.length === 0) {
            ...
            const hasAccountFields = Object.keys(cfg).some(
                (k) => !metaKeys.has(k),
            );
            if (hasAccountFields) {
                const id = normalizeId(
                    typeof cfg.id === "string" ? cfg.id : "default",
                );
                const enabled =
                    cfg.enabled === undefined ? true : cfg.enabled === true;
                const directory =
                    typeof cfg.directory === "string"
                        ? cfg.directory.trim()
                        : undefined;
                result.push({
                    id,
    ...
    const channels: ChannelIdentity[] = channelIdentities.map((identity) => {
        ...
        return {
            ...
            enabled:
                ...
                parseBoolean(env[envKey], channelEnabledDefault),
        };
    });
