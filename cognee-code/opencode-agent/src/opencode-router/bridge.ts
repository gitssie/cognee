...
type BridgeDeps = {
  ...
  /** Abstract provider for OpenCode clients (shared server or sandbox). */
...
export async function startBridge(config: Config, logger: Logger, reporter?: BridgeReporter, deps: BridgeDeps = {}) {
  ...
  // ── OpenCode client provider (shared server or per-sandbox) ──────────
  const provider = deps.provider ?? null;

  /** Obtain a client handle for the given session identity. */
  async function getSessionHandle(
    ...
    if (provider) {
      return provider.getClientForSession(channel, identityId, peerKey, directory);
    }
    // Fallback when no provider is injected: use the legacy directory-scoped client.
    return {
      client: getClient(directory),
      release: async () => {},
    };
  }
  ...
  async function refreshHealth() {
    // When an abstract provider is present, delegate health to it.
    if (provider) {
      const health = await provider.getHealth();
      opencodeHealthy = health.healthy;
      opencodeVersion = health.version ?? "provider";
      if (healthTimer && healthIntervalMs !== HEALTH_SLOW_INTERVAL_MS) {
  ...
  async function stopActiveRun(
    ...
    await handle.client.session.abort({ sessionID });
  }
  ...
  if (inbound.fromMe) {
    ...
    // Provision inbound media files into the workspace so the OpenCode
    // client can access them. In sandbox mode this syncs into the VM;
    // in classic mode this does a local rename.
      ...
      if (filePaths.length > 0) {
        try {
          const moved = provider
            ? await provider.provisionFiles(filePaths, boundDirectory, inbound.channel, inbound.identityId, peerKey)
            : await mediaStore.relocateInboundFiles(filePaths, join(boundDirectory, ".opencode-router", "media"));
          for (const m of mediaParts) {
            const src = m.filePath?.trim();
            if (src && moved.has(src)) {
              m.filePath = moved.get(src);
              logger.debug({ src, dst: m.filePath }, "media: provisioned file to workspace");
            }
          }
        } catch (err) {
          logger.warn({ err }, "media: failed to provision files");
        }
      }
    }
    ...
    // Delegate event subscription to the provider.  In sandbox mode
    // the provider's cleanup loop handles activity tracking; in shared
    // server mode the bridge subscribes to SSE events per directory.
    if (provider) {
      provider.ensureEventSubscription(boundDirectory);
    } else {
      ensureEventSubscription(boundDirectory);
    }
    ...
    enqueue(key, async () => {
      ...
      try {
        ...
        if (!reply && !parts.some((part) => part.type === "tool")) {
          // Cold-start: sandbox may need extra warmup. Retry up to 3 times
          // with backoff to avoid "no visibleText" on first request.
          for (let retry = 0; retry < 3 && !reply; retry++) {
            logger.warn({ sessionID, retry }, "prompt returned no visible text; retrying");
  ...
  async function handleCommand(
    ...
    if (command === "stop") {
      ...
      await stopActiveRun(directory, session.session_id, channel, identityId, peerKey);
    ...
    if (command === "compact") {
      ...
      const directory = session.directory?.trim() || resolveIdentityDirectoryStr(channel, identityId) || defaultDirectory;
      await compactSession(directory, session.session_id, channel, identityId, peerKey);
