...
import { SSEListener, ReactiveSSEListener } from "../sse-listener.js";
import type {
...
export const E2B_WORKSPACE = "/workspace";
...
class E2BSandboxInstance implements SandboxRuntime {
    ...
    // ── SSE listener (reactive: follows client changes across sandbox restarts) ─
    /**
     * A single ReactiveSSEListener per instance. When the sandbox restarts and
     * `this.client` is replaced, we call `_reactiveSSE.setClient(newClient)` and
     * it automatically tears down the old SSE connection, opens a new one with
     * the fresh client, and re-registers all existing session subscriptions.
     *
     * On stop/dispose we call `_reactiveSSE.setClient(null)` to close the
     * connection without destroying the subscriber registry (subscribers will be
     * re-connected on the next `setClient(newClient)` call).
     */
    private readonly _reactiveSSE: ReactiveSSEListener;

    private get sseListener(): ReactiveSSEListener {
        return this._reactiveSSE;
    }
    ...
    constructor(
        ...
        // Initialize the reactive SSE handle. It has no client yet — setClient()
        // is called once the sandbox is up and this.client is assigned.
        this._reactiveSSE = new ReactiveSSEListener({
            ...
            onIdle: () => {
                ...
                this.stop("idle").catch((err) =>
                    logger?.warn({ err, identity }, "stopRuntime(idle) failed"),
                );
            },
        });
    }
        ...
        return Effect.suspend(() => {
            ...
            if (this.sandbox !== null && this.client) {
                ...
                return Effect.tryPromise(
                    async (): Promise<SandboxConnection> => {
                        const alive = await sb.isRunning();
                        if (alive) {
                            this.logger?.info(
                                { sandboxId: this.sandboxId },
                                "reuse running e2b sandbox",
                            );
                        ...
                        this.logger?.warn(
                            { sandboxId: this.sandboxId },
                            "sandbox no longer running; clearing stale handle",
                        );
                        this.sandbox = null;
                        this.client = null;
                        this.baseUrl = "";
    ...
    private clearRuntimeState(dispose: boolean): Effect.Effect<void> {
        return Effect.sync(() => {
            // Disconnect SSE but keep the subscriber registry intact so listeners
            // are automatically re-connected when the sandbox restarts.
            this._reactiveSSE.setClient(null);
            this.sandbox = null;
            this.client = null;
            this.baseUrl = "";
    ...
    private async createSandboxAsync(): Promise<SandboxConnection> {
        ...
        if (this.cfg.hostMountEnabled && this.workspaceHostPath) {
            metadata["host-mount"] = JSON.stringify([
                { hostPath: this.workspaceHostPath, mountPath: E2B_WORKSPACE },
            ]);
        }
    ...
    private async reconnectSandboxAsync(): Promise<SandboxConnection> {
        ...
        this.logger?.info(
            { sandboxId: sid },
            "sandbox reachable; waiting for opencode",
        );
    ...
    private attachAndAwaitOpencode(sb: Sandbox): Promise<SandboxConnection> {
        ...
        this.client = client;
        // Notify the reactive SSE handle about the new client. It will stop the
        // old SSE connection (if any) and open a fresh one, then re-register all
        // existing session subscriptions automatically.
        this._reactiveSSE.setClient(client);
    ...
    private writeWorkspaceFilesEffect(
        ...
        return Effect.suspend(() => {
            ...
            for (const s of this.cfg.secrets) {
                ...
                if (provider && s.value)
                    auth[provider] = { type: "api", key: s.value };
            }
            ...
            return Effect.tryPromise(async () => {
                ...
                // db.json: opencode database connection config.
                // TODO: make this configurable via opencode-router.json
                await sb.files.write(
                    E2B_OPENCODE_PATHS.dataDir + "/db.json",
                    JSON.stringify(
                        {
                            url: "postgresql://user_local:dev-token-12345@172.16.20.37:5432/opencode",
                        },
                        null,
                        2,
                    ) + "\n",
    ...
    private connection(): SandboxConnection {
        return {
            ...
            events: () => {
                throw new Error("use sseListener instead of events()");
            },
            release: async () => {},
        };
    }
