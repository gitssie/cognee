  ...
  <q-page class="row no-wrap">
        ...
        <div class="column full-height bg-grey-1">
           <div class="q-pa-md row items-center justify-between">
             ...
             <q-btn round flat icon="add" color="primary" @click="showCreateDialog = true">
                <q-tooltip>New Notebook</q-tooltip>
             </q-btn>
           </div>
           <q-separator />
           <q-list separator v-if="!loading">
             ...
             <div v-if="notebooks.length === 0" class="text-center q-pa-md text-grey">No notebooks found</div>
           </q-list>
      ...
      <template v-slot:after>
        <div v-if="currentNotebook" class="full-height column bg-white">
           ...
           <div class="q-pa-md border-bottom row items-center q-gutter-sm bg-grey-1">
              ...
              <q-space />
              <q-btn flat icon="add" label="Markdown" @click="addCell('markdown')" color="primary" />
              <q-btn flat icon="code" label="Code" @click="addCell('code')" color="primary" />
           </div>
           ...
           <q-scroll-area class="col q-pa-md bg-grey-2">
             <div v-for="(cell, index) in currentNotebook.cells" :key="cell.id" class="q-mb-md">
                <q-card flat bordered class="bg-white">
                   ...
                   <q-bar class="bg-grey-3" dense>
                      ...
                      <q-space />
                      <q-btn v-if="cell.type === 'code'" flat dense icon="play_arrow" color="green" label="Run" @click="runCell(cell.id)" :loading="runningCells.has(cell.id)" />
                   ...
                   <q-card-section class="q-pa-none">
                      <template v-if="cell.type === 'markdown'">
                         ...
                         <div class="column">
                            ...
                            >
                               <MarkdownRender :content="cell.content || '*Double click to edit*'" />
                            </div>
                            <div v-if="editingCell === cell.id" class="row justify-end q-pa-xs bg-grey-3">
                               <q-btn flat dense label="Done" size="sm" color="primary" @click="editingCell = null; saveNotebook()" />
                            </div>
                         </div>
                      </template>
             ...
             <div v-if="currentNotebook.cells.length === 0" class="text-center text-grey q-mt-xl">
                 ...
                 <div class="text-subtitle1 q-mt-md">Add a cell to start</div>
             </div>
           </q-scroll-area>
        </div>
        ...
        <div v-else class="full-height row flex-center bg-grey-1 text-grey-6 column">
           ...
           <div class="text-h5 q-mt-md">Select a Notebook</div>
           <q-btn unelevated color="primary" label="Create Notebook" class="q-mt-lg" icon="add" @click="showCreateDialog = true" />
        </div>
      </template>
    </q-splitter>
    ...
    <q-dialog v-model="showCreateDialog">
       <q-card style="min-width: 300px">
          <q-card-section>
             <div class="text-h6">New Notebook</div>
          </q-card-section>
          <q-card-section>
             <q-input v-model="newNotebookName" label="Name" autofocus @keyup.enter="createNotebook" />
          </q-card-section>
          <q-card-actions align="right">
             <q-btn flat label="Cancel" v-close-popup />
             <q-btn color="primary" label="Create" @click="createNotebook" />
          </q-card-actions>
       </q-card>
    </q-dialog>
  </q-page>
...
async function loadNotebooks() {
  ...
  try {
    ...
    $q.notify({ type: 'negative', message: 'Failed to load notebooks' });
  } finally {
    loading.value = false;
  }
}
...
async function createNotebook() {
  ...
  try {
    ...
    $q.notify({ type: 'positive', message: 'Notebook created' });
  } catch {
    $q.notify({ type: 'negative', message: 'Failed to create notebook' });
  }
}
    ...
    $q.dialog({
      ...
      void (async () => {
        try {
          ...
          await loadNotebooks();
          $q.notify({ type: 'positive', message: 'Notebook deleted' });
        } catch {
          $q.notify({ type: 'negative', message: 'Failed to delete notebook' });
        }
      })();
    });
}
...
async function saveNotebook() {
    ...
    try {
        ...
        $q.notify({ type: 'negative', message: 'Failed to save notebook' });
    }
}
...
function addCell(type: 'markdown' | 'code') {
    ...
    const newCell: NotebookCell = {
        ...
        type,
        name: type === 'code' ? 'Code Cell' : 'Markdown Cell',
        content: ''
    };
